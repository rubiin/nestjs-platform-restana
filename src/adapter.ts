...
import { ServeStaticOptions } from './interface/serve-static-option.interface';

export type VersionedRoute = <
  Tany extends Record<string, any> = any,
  TResponse = any,
>(
  req: Tany,
  res: TResponse,
...
export class RestanaHttpAdapter extends AbstractHttpAdapter {
  ...
  public useStaticAssets(path: string, options: ServeStaticOptions) {
    const serve = serveStatic(path, options);
    if (options?.prefix) {
  ...
  public registerParserMiddleware(prefix?: string, rawBody?: boolean) {
    const bodyParserJsonOptions =
      this.getBodyParserOptions<OptionsJson>(rawBody);
    const bodyParserUrlencodedOptions =
      this.getBodyParserOptions<OptionsUrlencoded>(rawBody, { extended: true });

    const parserMiddleware = {
      jsonParser: bodyParserJson(bodyParserJsonOptions),
      urlencodedParser: bodyParserUrlencoded(bodyParserUrlencodedOptions),
    };
    Object.keys(parserMiddleware).forEach((parserKey) =>
  ...
  getBodyParserOptions<ParserOptions extends Options>(
    ...
    const rawBodyParser = (
      req: RawBodyRequest<http.IncomingMessage>,
